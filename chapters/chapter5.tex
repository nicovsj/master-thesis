...
variables that are opened and closed \emph{before} reading the $i$-th letter. In
particular, when $S = \emptyset$, the automaton changes the state and nothing is
output.
...
$q$ to~$q'$ that outputs $S$ when reading a letter $a$, and use $*$ to denote any
...
\emph{deterministic} $\EVA$s. We say that an $\EVA$ $\cE$ is \emph{deterministic}
...
A deterministic $\EVA$ ensures that for every document and output sequence, %TODO: proof
...
same output for every document), and that $\cE^{\odet}$ is deterministic. % TODO: proof

An inconvenience of $\cE^{\odet}$ is that its size can be exponential in $|\cE|$.
Fortunately, for the evaluation algorithm it is not necessary to construct % TODO: reference
...
\emph{set-lists} that store state-sets, and \emph{nodes} created and operated
...
symbol, from $i=0$ to $n$ (line 4). During the $i$-th iteration,
$\cstateslists$ keeps all state-sets $X$ that can be reached by runs reading
$b_0\ldots b_{i-1}$, and $X.\coid$ is the node storing all outputs of these runs.
