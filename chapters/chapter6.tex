...
expression and converts it to a logical VA (lVA), which is used both by the
...
\begin{enumerate}
	...
	English literature greats: Mark Twain, William Shakespeare, and Charles
	Dickens. We used texts provided by the Project
	Gutenberg\footnote{\url{https://www.gutenberg.org/}}, and concatenated them
	...
	the BLAST initiative~\cite{blast}. The combined size of this dataset is 38.5
	MB and 38.5 million characters.
	...
	by the Linked SPARQL Queries Dataset team \cite{LSQ}. We merged all the logs
	into a single document weighing 71.1 MB, and consisting of roughly 76
	million characters.
\end{enumerate}
...
\begin{enumerate}
	...
	provided by \cite{morphemes}. We then specify queries that look for
	2-grams~\cite{morphemes}, that is, two consecutive words each containing a
	...
	database~\cite{prosite} that commonly occur in our dataset. Our queries take
	any such pair of motifs, and look for their occurrences in the proteomic
	...
	keywords~\cite{sparql11} (e.g., \texttt{WHERE} or \texttt{OPTIONAL}), and
...
operators, and the ones that cannot. In our comparison the following engines are
used:
\begin{itemize}
	\item \textsf{PCRE}~\cite{pcre} version: 8.45;
	\item \textsf{PCRE2}~\cite{pcre2} version 10.40 (using JPCRE2 C++
	wrapper~\cite{wrapper});
...
\subsection{Discussion}
As we can see, \rematch\ shows good performance as compared to other engines. On
...
When it comes to \textsf{SPARQL} the story is similar. We remark that in this
...
more work compared to other engines. This is particularly evident when
comparing to engines not using look-around operators (\textsf{RE2} and
\textsf{TRE}). Even for the engines with look-around supported, we sometimes
cannot capture all the outputs (for instance when two nested matches start at
the same position), as witnessed in Table~\ref{tab:outputs}. Overall, the
experiments illustrate that the task of encountering \emph{all} the outputs,
including overlapping ones, can be done with minimal overhead when comparing
with classical RegEx matching.
