...
\begin{example}\label{example:logicalVA}
	Consider the REQL query \texttt{e0} of Example~\ref{example:basic}. The following is a logical VA representing \texttt{e0}:
	\begin{center}
...
Offsets are implemented in the rewriting module of \rematch\ after constructing the first logical VA from a REQL query. When the query contains quantifiers or alternations special care must be taken for offsetting the variables. More details are provided in Appendix \ref{app:offset}.
