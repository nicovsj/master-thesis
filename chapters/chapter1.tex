...
framework~\cite{FaginKRV15} that captures all appearances of a pattern in the
document. 
...
in~\cite{FaginKRV15}. For us, a document $d$ is simply a string over some finite
...
$0$)\footnote{In \cite{FaginKRV15}, the first position is $1$. We use $0$ to be
...
We say that a REQL query is \emph{well-designed}\footnote{In~\cite{FaginKRV15},
...
Note that, as shown in~\cite{FaginKRV15}, the well-designed condition does not
...
REQL, similarly as in \cite{MaturanaRV17}. This is done in
